...
class Order extends React.Component {
    ...
    render() {
        return (
            <div>
                ...
                >
                    {this.state.loading || !this.state.currentProduct ? (<div>loading...</div>) : (
                        <div>
                            <img src={this.state.currentProduct.photoForOrder} style={{ width: '85px', height: '72px', marginLeft: '5px', display: 'inline-block' }} />
                            <div>
    ...
    renderProduct() {
        ...
        this.state.products.map((product, i) => {
            tmp.push(
                <Card style={{ width: '100%', paddingBottom: '0px' }} className="order-card">
                    <div style={{ display: 'flex', marginTop: '-10px' }}>
                        <div className="uk-first-column">
                            <img src={product.photoForOrder} style={{ width: '85px', height: '72px', marginLeft: '15px' }} />
                        </div>
                        <div style={{ marginLeft: '20px', textAlign: 'left' }}>
                            <p style={{ textAlign: 'left', fontSize: '18px', fontWeight: 500, marginTop: '-5px' }}>{product.productNm}</p>
                            <p style={{ fontSize: '0.875rem', lineHeight: 1.4, color: '#999', marginTop: '-17px' }}>{product.description}</p>
                            <p style={{ marginBottom: '-14px', fontSize: '16px', marginTop: '-10px' }}>{product.price} đ</p>
                            <div className="uk-position-bottom-right">
                                <span uk-icon="icon: plus-circle; ratio: 1.5" className="uk-text-primary uk-icon" style={{ marginLeft: "450px" }} onClick={() => this.setModalVisibleAndProduct(true, product)}>
                                    <svg width="30" height="30" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <circle fill="none" stroke="#000" stroke-width="1.1" cx="9.5" cy="9.5" r="9" style={{ color: '#ea8025 !important' }}></circle>
                                        <line fill="none" stroke="#000" x1="9.5" y1="5" x2="9.5" y2="14" style={{ color: '#ea8025 !important' }}></line>
                                        <line fill="none" stroke="#000" x1="5" y1="9.5" x2="14" y2="9.5" style={{ color: '#ea8025 !important' }}></line>
                                    </svg>
                                </span>
                            </div>
                        </div>
