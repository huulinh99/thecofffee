        ...
        this.props.productForOrder.map((product, i) => {
            tmp.push(
                <Card style={{ height: '70px', marginTop: '20px' }}>
                    <div onClick={() => this.props.onClick(product)}>
                        <div style={{ display: 'inline' }}>
                            <span style={{
                                marginLeft: '-290px', padding: ' 0 10px', background: '#ea8025', lineHeight: '1.5',
                                fontSize: '0.875rem', verticalAlign: 'middle', whiteSpace: 'nowrap', borderRadius: '2px', textTransform: 'uppercase', color: '#ffffff'
                            }}>{product.quantity}</span>
                        </div>
                        <div style={{ display: 'inline' }}>
                            <p style={{
                                fontWeight: 'bold', fontSize: '20px', marginTop: '-26px',
                                marginLeft: '20px', fontFamily: 'Open Sans', lineHeight: 1.5
                            }}>{product.name}</p>
                            <p style={{ marginTop: '-43px', marginLeft: '265px', lineHeight: 1.5, fontSize: '16px' }}>{product.price}đ</p>
                        </div>
